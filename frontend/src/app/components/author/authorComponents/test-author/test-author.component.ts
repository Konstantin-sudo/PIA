...
@Component({
  selector: 'app-test-author',
  templateUrl: './test-author.component.html',
  styleUrls: ['./test-author.component.css']
})
export class TestAuthorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  initPage(setTime) {
    let testName = JSON.parse(sessionStorage.getItem('currTestNameAuthor'));

    this.currQuestion = { type: "" };
    this.questions = new Array<number>();

    this.authorService.getTest(testName).subscribe((user) => {
      this.test = user[0];
  ...
  endTest() {
    ...
    //to DB
    this.authorService.endTest(this.test).subscribe(testRes => {
      console.log("testRes");
      console.log(testRes);
      this.authorService.updateUser(user).subscribe(userRes => {
        console.log("userRes");
        console.log(userRes);
        this.minutesLeft = 0;
        this.secondsLeft = 0;
