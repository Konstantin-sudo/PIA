...
export class TestDetailsComponent implements OnInit {
  ...
  constructor(private router: Router, private app: AppComponent, private authService: AuthenticationService, private clientService: ClientService) { }

  ngOnInit() {
    if (this.authService.check("client")) {
      let currTestName = JSON.parse(sessionStorage.getItem('currTestNameClient'));
      this.clientService.getTest(currTestName).subscribe((test) => {
  ...
  fill() {
    this.router.navigate(['client/tests/test']);
  }
  goBack() {
